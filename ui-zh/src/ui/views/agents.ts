...
const TOOL_SECTIONS = [
  ...
  },
  {
    ...
    label: "运行时",
    tools: [
      { id: "exec", label: "exec", description: "运行Shell命令" },
  ...
  },
  {
    ...
    tools: [
      ...
      { id: "sessions_spawn", label: "sessions_spawn", description: "生成子智能体" },
...
function buildAgentContext(
  ...
  return {
    ...
    identityName,
    identityEmoji,
    skillsLabel: skillFilter ? `${skillCount} 个已选择` : "所有技能",
      ...
      <section class="agents-main">
        ${
          !selectedAgent
            ? html`
                <div class="card">
                  <div class="card-title">选择一个智能体</div>
                  <div class="card-sub">选择一个智能体以查看其工作区和工具。</div>
                </div>
              `
            : html`
              ${renderAgentHeader(
      ...
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">网关范围内的渠道状态快照。</div>
          </div>
...
function renderAgentCron(params: {
  ...
  return html`
    ...
    <section class="card">
      ...
      <div class="card-sub">计划给此智能体的任务。</div>
      ${
        jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">未分配定时任务。</div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
                    ...
                    !activeEntry
                      ...
                      : html`
                          ...
                          ${
                            activeEntry.missing
                              ? html`
                                  <div class="callout info" style="margin-top: 10px">此文件缺失。保存将在智能体工作区中创建它。</div>
                                `
                              : nothing
                          }
        ...
        hasAgentAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                此智能体在配置中使用了显式白名单。工具覆盖在配置选项卡中管理。
              </div>
            `
          : nothing
      }
...
function renderAgentSkills(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">技能权限</div>
          <div class="card-sub">
            此智能体的技能白名单。
            ${totalCount > 0 ? html`<span class="mono">${enabledCount}/${totalCount}</span> 已启用。` : nothing}
          </div>
        </div>
        <div class="row" style="gap: 8px;">
      ...
      ${
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">加载网关配置以设置每智能体技能。</div>
            `
          : nothing
      }
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">此智能体使用自定义技能白名单。</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
                所有技能已启用。禁用任何技能将创建每智能体白名单。
              </div>
            `
      }
      ${

...
export function renderGoogleChatCard(params: {
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">聊天 API Webhook 状态和渠道配置。</div>
      ${accountCountLabel}
      ...
      <div class="status-list" style="margin-top: 16px;">
        ...
        <div>
          ...
          <span>${googleChat?.credentialSource ?? "不适用"}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span>${googleChat?.lastStartAt ? formatAgo(googleChat.lastStartAt) : "不适用"}</span>
        </div>
        <div>
          <span class="label">上次探测</span>
          <span>${googleChat?.lastProbeAt ? formatAgo(googleChat.lastProbeAt) : "不适用"}</span>
        </div>
      </div>

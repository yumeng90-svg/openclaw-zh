...
export function renderNode(params: {
  ...
  const keyStr = String(path.at(-1));
  // 优先尝试翻译 hint.label (保持上下文特异性)，然后是 keyStr
  let label =
    (hint?.label ? lookupTranslation(hint.label) : undefined) ??
    lookupTranslation(keyStr) ??
    hint?.label ??
    schema.title ??
    humanize(keyStr);

  // 如果最终的 label 仍然可能是英文（例如来自 schema.title），尝试直接翻译
  if (label) {
    const directLabel = lookupTranslation(label);
    if (directLabel) {
      label = directLabel;
    }
  }

  // 优先尝试通过 (label + "_help") 或 (key + "_help") 查找帮助文本翻译
  const helpKey = (hint?.label ?? keyStr) + "_help";
  let help = lookupTranslation(helpKey) ?? hint?.help ?? schema.description;

  // 如果通过 key 没找到翻译，尝试直接翻译内容 (针对 description 本身就在翻译表中的情况)
  if (help && !lookupTranslation(helpKey)) {
    const directHelp = lookupTranslation(help);
    if (directHelp) {
      help = directHelp;
    }
  }
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">不支持的模式节点。请使用 Raw 模式。</div>
    </div>`;
  }
          ...
          <div class="cfg-segmented">
            ${literals.map(
              ...
              >
                ${
                  // oxlint-disable typescript/no-base-to-string
                  lookupTranslation(String(lit)) ?? String(lit)
                }
              </button>
            `,
            )}
        ...
        <div class="cfg-field">
          ...
          <div class="cfg-segmented">
            ${options.map(
              ...
              >
                ${lookupTranslation(String(opt)) ?? String(opt)}
              </button>
            `,
            )}
          </div>
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error">不支持的类型: ${type}。请使用 Raw 模式。</div>
    </div>
  `;
}
...
function renderTextInput(params: {
  ...
  const hint = hintForPath(path, hints);
  const keyStr = String(path.at(-1));
  let label = hint?.label ?? lookupTranslation(keyStr) ?? schema.title ?? humanize(keyStr);
  if (label) {
    const directLabel = lookupTranslation(label);
    if (directLabel) {
      label = directLabel;
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        ...
        ${options.map(
          (opt, idx) => html`
          <option value=${String(idx)}>${lookupTranslation(String(opt)) ?? String(opt)}</option>
        `,
        )}
      </select>
    </div>
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">不支持的数组模式。请使用 Raw 模式。</div>
      </div>
    `;
  }

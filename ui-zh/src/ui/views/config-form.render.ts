...
export const SECTION_META: Record<string, { label: string; description: string }> = {
  ...
  approvals: { label: "审批", description: "人工审批策略" },
  nodeHost: { label: "节点主机", description: "节点主机配置" },
  diagnostics: { label: "诊断", description: "系统诊断和追踪配置" },
};
...
export function renderConfigForm(props: ConfigFormProps) {
  ...
  if (schemaType(schema) !== "object" || !schema.properties) {
    return html`
      <div class="callout danger">不支持的 Schema。请使用源码模式。</div>
    `;
  }
    ...
    return html`
      <div class="config-empty">
        ...
        <div class="config-empty__text">
          ${searchQuery ? `没有找到匹配 "${searchQuery}" 的设置` : "此部分无设置"}
        </div>
      </div>
    `;
  }
      ...
      ${
        subsectionContext
          ? (() => {
              ...
              let label = hint?.label ?? node.title ?? humanize(subsectionKey);
              const directLabel = lookupTranslation(label);
              if (directLabel) {
                label = directLabel;
              }

              let description = hint?.help ?? node.description ?? "";
              const directDesc = lookupTranslation(description);
              if (directDesc) {
          ...
          : filteredEntries.map(([key, node]) => {
              let meta = SECTION_META[key];
              if (!meta) {
                const label = lookupTranslation(key) ?? humanize(key);
                const description =
                  lookupTranslation(node.description ?? "") ?? node.description ?? "";

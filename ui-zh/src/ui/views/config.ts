...
function resolveSubsections(params: {
  ...
  const entries = Object.entries(schema.properties).map(([subKey, node]) => {
    ...
    const label = hint?.label ?? lookupTranslation(subKey) ?? node.title ?? humanize(subKey);
...
export function renderConfig(props: ConfigProps) {
  ...
  const validityClass = props.valid == null ? "" : props.valid ? "pill--ok" : "pill--danger";
  ...
  const extraSections = Object.keys(schemaProps)
    ...
    .map((k) => {
      ...
      return {
        key: k,
        label: meta?.label ?? lookupTranslation(k) ?? k.charAt(0).toUpperCase() + k.slice(1),
      };
    });
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          ...
          <span
            class="pill pill--sm ${validityClass}"
            >${validity}</span
          >
        </div>
                  ...
                  formUnsafe
                    ? html`
                        <div class="callout danger" style="margin-top: 12px">
                          表单视图无法安全编辑某些字段。使用 Raw 模式以避免丢失配置条目。
                        </div>
                      `
                    : nothing
                }

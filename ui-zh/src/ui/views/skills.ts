...
function formatInstallLabel(label: string): string {
  return label
    .replace(/^Install\s+(.+)\s+via\s+(.+)$/, "通过 $2 安装 $1")
    .replace(/^Install\s+/, "安装 ");
}
...
function renderSkill(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const missing = [
    ...skill.missing.bins.map((b) => `二进制:${b}`),
        ...
        <div class="row" style="justify-content: flex-end; flex-wrap: wrap;">
          ...
          ${
            canInstall
              ...
              >
                ${busy ? "安装中..." : formatInstallLabel(skill.install[0].label)}
              </button>`
              : nothing
          }
        </div>

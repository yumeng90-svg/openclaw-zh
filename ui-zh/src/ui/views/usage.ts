    ...
    return html`
      <div class="card usage-mosaic">
        <div class="usage-mosaic-header">
          <div>
            <div class="usage-mosaic-title">时间活动分布</div>
            <div class="usage-mosaic-sub">需要会话时间戳才能估算。</div>
          </div>
          <div class="usage-mosaic-total">${formatTokens(0)} token</div>
        </div>
        <div class="muted" style="padding: 12px; text-align: center;">暂无时间轴数据。</div>
      </div>
    `;
  }
      ...
      <div class="usage-mosaic-header">
        <div>
          ...
          <div class="usage-mosaic-sub">
            根据会话跨度估算（首次/最后活动）。时区：${timeZone === "utc" ? "UTC" : "本地"}。
          </div>
        </div>
        <div class="usage-mosaic-total">${formatTokens(stats.totalTokens)} token</div>
      </div>
        ...
        <div class="usage-mosaic-section">
          ...
          <div class="usage-hour-grid">
            ${stats.hourTotals.map((value, hour) => {
              ...
              const title = `${hour}:00 · ${formatTokens(value)} token`;
            ...
            return html`
              ...
              >
                ...
                <div class="daily-bar-tooltip">
                  ...
                  ${formatTokens(d.totalTokens)} token<br />
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? "Token" : "成本"} (按类型)</div>
...
function renderUsageInsights(
  ...
  const throughputHint = "吞吐量显示活跃时间内的每分钟 token 数。越高越好。";
  const tokensHint = "此范围内的每条消息平均 token 数。";
  const costHint = showCostHint
    ? "当提供商报告成本时的每条消息平均成本。此范围内部分或所有会话缺少成本数据。"
    : "当提供商报告成本时的每条消息平均成本。";
  ...
  const errorDays = aggregates.daily
    ...
    .map((day) => {
      ...
      return {
        ...
        sub: `${day.errors} 错误 · ${day.messages} 消息 · ${formatTokens(day.tokens)}`,
        rate,
      };
    })
    .toSorted((a, b) => b.rate - a.rate)
    ...
    <section class="card" style="margin-top: 16px;">
      ...
      <div class="usage-summary-grid">
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            工具调用
            <span class="usage-summary-hint" title="跨会话的总工具调用计数。">?</span>
          </div>
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">跨越 ${aggregates.messages.total || 0} 条消息</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            平均成本 / 消息
          ...
          <div class="usage-summary-sub">${formatCost(totals.totalCost)} 总计</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            会话
            <span class="usage-summary-hint" title="范围内的不同会话。">?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">共 ${totalSessions} 个 (在此范围内)</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            吞吐量
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${formatTokens(totals.cacheRead)} 缓存 · ${formatTokens(cacheBase)} 提示词
          </div>
        </div>
      </div>
      <div class="usage-insights-grid">
        ...
        ${renderPeakErrorList("错误高峰时段", errorHours, "无错误数据")}
      </div>
    </section>
  `;
}
    ...
    <div class="card sessions-card">
      <div class="sessions-card-header">
        ...
        <div class="sessions-card-count">
          ${sessions.length} 已显示${totalSessions !== sessions.length ? ` · ${totalSessions} 总计` : ""}
        </div>
      </div>
  ...
  return html`
    ...
    <div class="session-summary-grid">
      ...
      <div class="session-summary-card">
        ...
        <div class="session-summary-meta">${usage.toolUsage?.uniqueTools ?? 0} 工具</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">错误</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">${usage.messageCounts?.toolResults ?? 0} 工具结果</div>
      </div>
      <div class="session-summary-card">
      ...
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${
            usage
              ? html`
            <span><strong>${formatTokens(usage.totalTokens)}</strong> token</span>
...
function renderTimeSeriesCompact(
  ...
  if (!timeSeries || timeSeries.points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">无时间轴数据</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="session-timeseries-compact">
      ...
      <div class="timeseries-summary">${points.length} 消息 · ${formatTokens(cumTokens)} · ${formatCost(cumCost)}</div>
      ${
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                ...
                <div class="cost-breakdown-legend">
                  ...
                  <div class="legend-item" title="写入缓存 Token">
                    <span class="legend-dot cache-write"></span>缓存写入 ${formatTokens(sumCacheWrite)}
                  </div>
                  <div class="legend-item" title="读取缓存 Token">
  ...
  return html`
    <div class="context-details-panel">
      ...
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title="系统: ~${formatTokens(systemTokens)}"></div>
        <div class="context-segment skills" style="width: ${pct(skillsTokens, totalContextTokens).toFixed(1)}%" title="技能: ~${formatTokens(skillsTokens)}"></div>
        <div class="context-segment tools" style="width: ${pct(toolsTokens, totalContextTokens).toFixed(1)}%" title="工具: ~${formatTokens(toolsTokens)}"></div>
        <div class="context-segment files" style="width: ${pct(filesTokens, totalContextTokens).toFixed(1)}%" title="文件: ~${formatTokens(filesTokens)}"></div>
      </div>
            ...
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    ...
                    ${
                      more > 0
                        ? html`<div class="context-breakdown-more">+${more} 更多</div>`
                        : nothing
                    }
                  </div>
                `;
      ...
      <div class="session-logs-list">
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">没有匹配筛选条件的消息。</div>
              `
            : nothing
        }
      </div>
...
export function renderUsage(props: UsageProps) {
  ...
  return html`
    ...
    <section class="usage-page-header">
      ...
      <div class="usage-page-subtitle">查看令牌去向、会话高峰时间和费用驱动因素。</div>
    </section>

    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
      <div class="usage-header-row">
        <div class="usage-header-title">
          <div class="card-title" style="margin: 0;">过滤器</div>
          ${
            props.loading
              ? html`
          ...
          ${
            isEmpty
              ? html`
                  <span class="usage-query-hint">选择日期范围并点击刷新以加载用量统计。</span>
                `
              : nothing
          }
        </div>
        <div class="usage-header-metrics">
          ...
          <button
            ...
            title=${props.headerPinned ? "取消固定过滤器" : "固定过滤器"}
      ...
      <div style="margin-top: 12px;">
          ...
          <input
            ...
            placeholder="过滤会话 (例如: key:agent:main:cron* model:gpt-4o has:errors minTokens:2000)"
          ...
          <div class="usage-query-actions">
            ...
            <span class="usage-query-hint">
              ${
                hasQuery
                  ? `${filteredSessions.length} / ${totalSessions} 个会话匹配`
